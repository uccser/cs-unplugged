...
upper_range = input('Enter a number as the upper range: ')
numbers_after_list = [1, 10, 100]
number = random.randint(1, upper_range)
numbers_after = numbers_after_list[random.randint(0, 2)]
answer = 0
if numbers_after == 1:
    answer = input('What is the number after ' + str(number) + '?')
else:
    answer = input('What is ' + str(numbers_after) + ' numbers after ' + str(number) + '?')
