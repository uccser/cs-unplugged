...
existing images with ``./dev wipe``, and then build and start the system with
``./dev start``.
...
filepath is correct.
When running ``./dev update``, the script will report an error if an image
cannot be found.
...
(``.svg`` instead of ``.txt``), try running ``./dev update``.
...
If you are editing SCSS files, check the ``./dev static`` command isn't reporting
SCSS compilation errors.

The website isn't displaying when I open ``cs-unplugged.localhost`` in a browser
----------------------------------------------------------------------------------------------

Check you have run the ``./dev start`` command, and that it hasn't reported any
errors.
